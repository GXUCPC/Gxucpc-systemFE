  ...
  <div>
    <div class="front">
      <el-card class="card">
        ...
        <el-form ref="download-form" :label-position="labelPosition" label-width="100px" :model="formLabelAlign"
                 :rules="rules"
          ...
          <el-form-item label="比赛编号" prop="itemNumber">
            ...
            <el-select v-model="formLabelAlign.itemNumber" filterable="true" remote="true"
                       placeholder="输入并选择比赛名称" :remote-method="remoteMethod" :loading="loading">
...
export default {
  ...
  methods: {
    ...
    async downloadFile(obj) {
      await this.$http.post("/public/download", obj, {responseType: 'blob'}).then(res => {
        const content = res;
    ...
    async submitForm(obj) {
      let vis = await this.$refs['download-form'].validate();
      if (!vis) return;
      ...
      await this.$http.post("/public/download/checkStatus", obj).then(res => {
        if (res.statusCode === 50000) {
